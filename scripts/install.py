...
def real_files(root: Path) -> Iterable[tuple[Path, Path]]:
    if root.is_symlink():
        raise ValueError(f"source must not be a symlink: {root}")
    if root.is_file():
...
def safe_destination(home: Path, relative: str) -> Path:
    ...
    while parent != home:
        if parent.is_symlink():
            raise ValueError(f"install target has a symlinked parent: {parent}")
...
def install(repo: Path, home: Path, desired: dict[str, dict[str, Any]], receipt_path: Path, product: str) -> tuple[int, int, Path | None]:
    ...
    for target_text in stale:
        ...
        if target.is_file() and not target.is_symlink() and digest(target) == prior.get("sha256"):
            target.unlink()

    for target_text, row in desired.items():
        target = safe_destination(home, str(Path(target_text).relative_to(home)))
        source = repo / row["source"]
        current_hash = digest(target) if target.is_file() and not target.is_symlink() else None
        ...
        if target.exists() or target.is_symlink():
            ...
            if current_hash != prior_hash:
                ...
                if target.is_symlink():
                    raise ValueError(f"refusing divergent managed symlink: {target}")
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--update", action="store_true", help="Reconcile managed copies to this checkout")
